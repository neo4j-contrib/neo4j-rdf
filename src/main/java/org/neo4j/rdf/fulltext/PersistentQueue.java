...
public class PersistentQueue implements Iterator<PersistentQueue.Entry>
{
	...
	public synchronized void markAsCompleted( Entry entry )
	{
		try
		{
			...
			try
			{
				...
				if ( ( Byte ) readNextEntryHeader( true )[ 0 ] == COMPLETED )
				{
					return;
				}
	...
	private synchronized Object[] readNextEntryHeader(
		boolean restorePositionAfterRead ) throws IOException
	{
		...
		try
		{
			...
			return new Object[] { state, entrySize };
		}
		finally
		{
	...
	private synchronized Entry tryToFindNext() throws IOException
	{
		...
		{
			long positionBeforeRead = channel.position();
			Object[] header = readNextEntryHeader( false );
			totalQueueIndex.incrementAndGet();
			byte state = ( Byte ) header[ 0 ];
			int entrySize = ( Integer ) header[ 1 ];
			...
			{
				channel.position( channel.position() + entrySize );
				continue;
			}
			else
			{
				throw new IOException( "Invalid entry state " + state );
			}
		}
		return result;
	}
